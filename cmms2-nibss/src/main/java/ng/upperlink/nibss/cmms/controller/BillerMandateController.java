...
public class BillerMandateController {
    ...
    //private Logger log = LoggerFactory.getLogger(BillerMandateController.class);
   ...
   // @Lazy
    ...
    public ResponseEntity createMandate(@Valid @RequestBody MandateRequest requestObject,
        ...
        mandateValidator.validate(requestObject,null);
...
@JsonView(Views.MandateView.class)
    public ResponseEntity test(){
        try{
            Mandate mandate = mandateService.getMandateByMandateId(22L);
            ObjectMapper mapper = new ObjectMapper();
            mapper.configure(DeserializationFeature.FAIL_ON_UNKNOWN_PROPERTIES, false);

            String objString = mapper.writeValueAsString(mandate);
            System.out.println(objString);

            Mandate m = mapper.readValue(objString,Mandate.class);
            System.out.println("********************");
