...
public class NibssController {
    ...
    //private final static Logger log = LoggerFactory.getLogger(NibssController.class);
...
//            //get the user to be updated
//            nibss = nibssUserService.get(request.getId());
//            if (nibss == null)
...
//
//       try{
//           return searchRepo.searchNIBBSUser(request.getRole(), request.getEmail(), request.getFirstName(),
...
//       }catch(Exception ex){
//           log.error("Search for NIBSS users failed ",ex);
//           return new ResponseEntity<>(new ErrorDetails(Errors.REQUEST_TERMINATED.getValue()), HttpStatus.INTERNAL_SERVER_ERROR);
//       }
//
//    }
