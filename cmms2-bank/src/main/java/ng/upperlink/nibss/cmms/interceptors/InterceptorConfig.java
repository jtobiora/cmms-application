...
public class InterceptorConfig extends HandlerInterceptorAdapter implements Filter{
  ...
  /*       if (!isUserAuthorized(task,(HandlerMethod) handler)){
    ...
    public void doFilter(ServletRequest servletRequest, ServletResponse servletResponse, FilterChain filterChain) throws IOException, ServletException {
        ...
        response.setHeader("Access-Control-Allow-Headers", "API-KEY, API_KEY, LANG, LANGUAGE, " +
                "User-Agent, Accept-Encoding, Origin, Accept, Accept-Language, Content-Language, " +
                "Content-Type, Authorization, x-forwarded-by, Last-Event-ID, DPR, Save-Data, Viewport-Width, Width");
        response.setHeader("Access-Control-Allow-Methods", "GET,HEAD,PUT,PATCH,POST,OPTIONS,DELETE");
        filterChain.doFilter((HttpServletRequest) servletRequest, response);
    }
